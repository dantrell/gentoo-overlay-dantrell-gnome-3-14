...
COMMON_DEPEND="
	...
	>=x11-libs/gtk+-3.7.8:3[X,wayland?]
...
src_prepare() {
	if use ck; then
		...
		eapply "${FILESDIR}"/${PN}-3.14.4-restore-deprecated-code.patch
	fi
	...
	eapply "${FILESDIR}"/${PN}-3.14.4-housekeeping-fix-improper-notify-notification-close-usage.patch
